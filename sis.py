        ...
        with open('data.csv','r') as csv_file:
                ...
                for line in read:
                        ...
                        if(line['id#'].startswith(searchword) or (line['name'].lower()).startswith(searchword.lower())):
                                ...
                                currFrame = Frame(myFrame, bg="white", highlightbackground="#B0C4DE", highlightthickness=3, height=100, width=470)
...
header = Frame(root, height=100, width=500, bg="#778899")
header.propagate(0)
header.grid(row=0, column=0)
Label(header, text="STUDENT INFORMATION SYSTEM", font = 'Helvitica 20 bold', bg="#778899", fg="#000000").place(relx=.5, rely=.4, anchor='c')
Label(header, text="a simple demonstration", font = 'Helvitica 12 italic', bg="#778899", fg="#000000").place(relx=.5, rely=.65, anchor='c')

#Code for a dynamic searchbar
searchFrame = Frame(root)
searchFrame.grid(row=1,column=0,pady=3)
Label(searchFrame, bg="#778899", text="SEARCH:", anchor=W, font = 'Arial 9 bold', fg="#000000").grid(row=0, column=0)
...
myFrame= Frame(mycanvas, bg="#778899")
...
add = Button(root,image=button_addstudent, borderwidth=0,bg='#778899', command=lambda:info_window("add",1))
